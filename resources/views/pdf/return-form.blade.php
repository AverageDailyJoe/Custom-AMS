...
<body>
    <div class="container">
        ...
        <div style="margin-bottom: 8px; font-size: 11px;">
            Pada hari ini tanggal <strong>{{ $checkout->checked_in_at ? $checkout->checked_in_at->translatedFormat('d F Y') : date('d F Y') }}</strong>, yang bertandatangan di bawah ini:
        </div>
        ...
        <table class="info-table">
            ...
            <tr>
                <th>PENERIMA (PIHAK KEDUA / IT)</th>
                <td><strong>{{ $checkout->checkedInByUser?->name ?? ($checkout->checkedOutByUser?->name ?? 'IT Staff') }}</strong> (Departemen Information & Technology)</td>
            </tr>
            <tr>
        ...
        <div class="terms-box">
            ...
            <p>1. PIHAK PERTAMA telah menyerahkan kembali 1 (satu) unit laptop/perangkat beserta aksesoris pendukungnya kepada PIHAK KEDUA dalam kondisi sebagaimana tercantum pada tabel pemeriksaan di atas.</p>
            <p>2. PIHAK KEDUA telah menerima dan melakukan pengecekan fisik serta pengujian fungsi teknis terhadap unit tersebut.</p>
            <p>3. Dalam hal terjadi kerusakan atau kekurangan komponen akibat kelalaian PIHAK PERTAMA selama masa penggunaan yang belum dilaporkan/diperbaiki, maka nilai kerugian akan diperhitungkan dan diselesaikan sesuai ketentuan <strong>PT Gondowangi Tradisional Kosmetika</strong>.</p>
            <p>4. PIHAK PERTAMA menyatakan bahwa seluruh data pribadi telah dibersihkan/di-backup dan data/aset perusahaan pada laptop telah diserahterimakan penuh.</p>
            <p>5. Dengan ditandatanganinya form ini, maka tanggung jawab atas penggunaan unit laptop resmi beralih kembali dari PIHAK PERTAMA kepada PIHAK KEDUA.</p>
        </div>

        <table class="sig-table">
            <tr>
                <td>
                    <p>Dibuat Oleh,<br><strong>PENGGUNA (PIHAK PERTAMA)</strong></p>
                    <div class="sig-space"></div>
                    <p><u>( {{ $checkout->primary_user ?? 'Pengguna Unit' }} )</u><br><small>{{ $checkout->asset->department ?? 'User' }}</small></p>
                </td>
                <td>
                    <p>Diterima Oleh,<br><strong>IT STAFF (PIHAK KEDUA)</strong></p>
                    <div class="sig-space"></div>
                    <p><u>( {{ $checkout->checkedInByUser?->name ?? ($checkout->checkedOutByUser?->name ?? 'IT Staff') }} )</u><br><small>Information Technology</small></p>
                </td>
                <td>
